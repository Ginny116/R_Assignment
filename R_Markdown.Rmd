...
## R Markdown
genotypes <- read_tsv ("C:/Users/giang nguyen/Desktop/R_Assignmnet (Review)/fang_et_al_genotypes.txt")
position <- read_tsv ("C:/Users/giang nguyen/Desktop/R_Assignmnet (Review)/snp_position.txt")

# Data Inspection
str(genotypes) # displays the internal structure of a given R object
nrow(genotypes) # return the number of rows _ [1] 2782 
ncol(genotypes) # return the numberof columns _ [1] 986 
dim(genotypes) # Dimensions of an Object _ [1] 2782  986
str(position)
nrow(position) [1] 983
ncol(position) [1] 15
dim(position) [1] 983  15
...
for (i in 1:2){if(i==1){
    ...
    print(pp)
    ggsave(pp,filename = "maize_plot.pdf",width = 5,height = 8)
    dev.off()
    getwd()
          }else{
            pp=ggplot(data = teosinte_genotypes) + 
             geom_bar(mapping = aes(x = Chromosome))
            print(pp)
            ggsave(pp,filename = "teosinte_plot.pdf",width = 5,height = 8)
            dev.off() }  }
    rm(i)
    rm(pp)
